...
import { TDipInfo, TDipContents, EnumDipType, EnumOprnOprtType, EnumDeviation, EnumInsuranceType, TDipContentsSupplement } from 'src/types/dip.type'
import { getCacheKey } from 'src/utils'
import { DipService } from '../dip.service'
import { RegionBaseService } from './region.base.service'

@Injectable()
export class Region_370900 extends RegionBaseService {
  ...
  toSettle(rawParams: DipTodo, formatParams: DipTodo, dipInfo: TDipInfo): TDipInfo {
    ...
    // 医疗机构系数区分职工/居民
    // 系数职工是0.611，居民是0.631
    configSettle.factorHospital = rawParams.insuranceType === EnumInsuranceType.职工 ? 0.611 : 0.631
    ...
    const getDipSettleFactorDeviation = () => {
      ...
      if (dipInfo.dipCode === 'KBBZ') {
        dipInfo.dipSettleDeviation = EnumDeviation.正常倍率
        return (sumAmount / dipInfo.dipAvgAmount) * 0.4368 * 0.9
      }
  ...
  intoComprehensiveGroup(rawParams: DipTodo, formatParams: DipTodo, dipContentList: TDipContents): TDipInfo[] {
    ...
    // 验证手术级别
    if (dipInfoList.some((item) => item.dipName.includes('(1)') || item.dipName.includes('(2)') || item.dipName.includes('(3)'))) {
